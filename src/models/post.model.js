...
const getByIdPosts = async (id) => {
  try {
    ...
    const idPost = "SELECT pb.usuario_id, pb.titulo, pb.descripcion, pb.created_at, u.nombre, pr.precio, pr.formato, pr.marca, pr.tipo, pr.imagen FROM publicaciones AS pb JOIN usuarios AS u ON pb.usuario_id = u.id JOIN productos AS pr ON pb.producto_id = pr.id WHERE pb.id = $1"
